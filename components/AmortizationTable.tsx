  ...
  if (interestType !== 'REDUCING') {
    return (
      <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
        <h2 className="text-xl sm:text-2xl font-bold mb-4 text-gray-800">{t.amortization.title}</h2>
        <p className="text-sm sm:text-base text-gray-600 text-center py-4">{t.amortization.notAvailable}</p>
      </div>
    );
  }

  const schedule = generateAmortizationSchedule(loanAmount, interestRate, durationYears);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800">{t.amortization.title}</h2>
        <button
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {schedule.map((row) => (
                    <tr key={row.month} className="hover:bg-gray-50">
                      <td className="px-3 sm:px-4 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 ltr:text-left rtl:text-right">{row.month}</td>
